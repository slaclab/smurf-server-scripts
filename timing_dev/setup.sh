...
function add_module_to_dockerfile() {
    ...
    echo "    "if [ -f configure/CONFIG_SITE.Common.rhel6-x86_64 ]";" then sed -i -e "'s|^PACKAGE_DIR=.*|PACKAGE_DIR=\${PACKAGE_SITE_TOP}|g'" configure/CONFIG_SITE.Common.rhel6-x86_64";" fi"; \\" >> Dockerfile
...
# No args
# Read through ./packages and get those packages from SLAC's filesystem.
function get_packages() {
    while read line; do
	echo Copying $line
	mkdir -p $temp_files_dir/packages/$line
	cp -Rn $package_dir/$line/rhel6-x86_64 $temp_files_dir/packages/$line
    done < packages
}
...
echo "Getting files."
cp Dockerfile_part1 Dockerfile
get_packages
get_epics
